...
import { TaskStatus, stopWorkLogging, TaskItem, getHistoryQueue, MAX_COMMAND_QUEUE_LENGTH, findCommon } from '../helpers/utils';
...
export const InputBox = props => {
    ...
    return <div className="bg-control w-full h-10 text-sm fixed bottom-0 left-0">
        <div className="w-full h-full relative h-8">
            <input ref={inputRef} className="bg-transparent w-full h-full p-2 px-3 absolute top-0 left-0 z-10" tabIndex={0} autoFocus={true} onKeyPress={processInput} onKeyUp={processInput} onKeyDown={onKeyDown} placeholder="Enter your command here. Press UP/DOWN for the previous commands." />
            <input ref={suggestRef} className="bg-transparent w-full h-full p-2 px-3 absolute top-0 left-0 z-0 pointer-events-none opacity-25" disabled={true} value={""} />
        </div>
    </div>;
};
